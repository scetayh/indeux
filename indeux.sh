...
if [ "$1" = "init" ]; then
    if [ ! -d "./.indeux" ]; then
        ...
        echo "* This directory is already inited for indeux. If necessary, operate \`sudo rm -rf ./.indeux/'."
        exit 1
    fi
elif [ "$1" = "version" ]; then
    Indeux_PrintBrand
    ...
    if [ ! -f "./.indeux/indeux.conf" ]; then
        echo "./.indeux/indeux.conf not found. Have you inited this directory for indeux?"
        exit 1
    fi
